...
static const RConfig factory_conf PROGMEM = {
    ...
    "%Id,%Rd,%Bd,%AB,%Cd",     ///< payload template
...
void init_config(bool reset){
    ...
    status.rf_mode=(rf_mode_t)get_cnf(CFG_RF_MODE);
        ...
        status.ttn_sf=get_cnf(CFG_TTN_CHANNEL);
...
#if defined(BOROSREED24)
const char PA_levelstr[5][5] PROGMEM = {"MIN","LOW","HIGH","MAX","ULT"};
const char Speedstr[3][8] PROGMEM = { "1MBPS", "2MBPS", "250KBPS"};
const char CRCstr[3][6] PROGMEM = { "None", "8bit", "16bit"};
#endif
...
void dump_config(Print &p){
    ...
    p.print(F("[repo]Report(s):")); p.println(get_cnfw(CFG_REPORT)*8);
    
    #if defined(BOROSREED24)

    if(get_cnf(CFG_RF_MODE) == RF_PLAIN) {
        p.println(F("[mode]:PLAIN"));
        p.print(F("==>Addr size:")); p.println(get_cnf(CFG_RF_ADDR_SIZE));
        p.print(F("==>Remote pipe:"));
        uint8_t addr[5]; 
        get_config(CFG_RF_DADDR,addr,5);
        print_hex(p,addr,get_cnf(CFG_RF_ADDR_SIZE));
        p.println();
        p.print(F("==>Payload size:")); p.println(get_cnf(CFG_RF_PAYLOAD_SIZE));
        p.print(F("==>AutoAck:")); p.println(get_cnf(CFG_RF_ACK));
        p.print(F("==>CRC:")); p.println(FH(CRCstr[get_cnf(CFG_RF_CRC)]));
        p.print(F("==>Retry Count:")); p.println(get_cnf(CFG_RF_RETRY));
        p.print(F("==>Retry delay (in us):")); p.println( (get_cnf(CFG_RF_RETRY_DELAY)+1)*250);

    } else {
        p.println(F("[mode]:MESH"));
        p.print(F("==>Mesh node ID:")); p.println(get_cnf(CFG_MSH_NODEID));
        p.print(F("==>Mesh frame type:0x")); p.println(get_cnf(CFG_MSH_FRAMEID),HEX);
        p.print(F("==>Mesh node ID:")); p.println(get_cnf(CFG_MSH_DADDR));
        p.print(F("==>Mesh force renew:")); p.println(get_cnf(CFG_MSH_FORCE_RENEW));
    }
     p.print(F("==>Tx power:")); p.println(FH(PA_levelstr[get_cnf(CFG_RF_PA_LEVEL)]));
     p.print(F("==>Data rate:")); p.println(FH(Speedstr[get_cnf(CFG_RF_SPEED)]));
     p.print(F("==>Channel:")); p.println(get_cnf(CFG_RF_CHANNEL));
    #elif defined(BOROSREED_LORA)
        uint8_t tmp[16];
        ...
        p.print(F("\n[por]Port:")); p.println(get_cnf(CFG_TTN_PORT));
